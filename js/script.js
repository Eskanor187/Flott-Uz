...
(function () {
  ...
  // Счёт → деньги: одна карточка проходит путь документа в выплату. Меняются
  // геометрия (портрет → альбом), цвет и два лица; счётчик дней идёт к нулю.
  var morphScene = document.getElementById('morphScene');
  if (morphScene) {
    var morphCard = document.getElementById('morphCard');
    var morphCaption = document.getElementById('morphCaption');
    var morphDoc = morphScene.querySelector('.morph-doc');
    var morphCash = morphScene.querySelector('.morph-cash');
    var morphCashSum = morphScene.querySelector('.morph-cash-sum');
    var morphRows = Array.prototype.slice.call(morphScene.querySelectorAll('.morph-rows span'));
    var mqMorph = window.matchMedia('(max-width: 880px)');
    var morphReduce = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    var morphTicking = false;
    var DAYS = 62;

    var clampM = function (v) { return Math.max(0, Math.min(1, v)); };
    var easeM = function (t) { return t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2; };

    // «через 62 дня» / «через 61 день» / «через 5 дней»
    var daysWord = function (n) {
      var t = n % 100, o = n % 10;
    ...
    // «вместо 62 дней» — родительный падеж, форма другая
    var daysGen = function (n) {
      return (n % 10 === 1 && n % 100 !== 11) ? 'дня' : 'дней';
    };

    var morphClear = function () {
      morphCard.style.width = '';
      morphCard.style.height = '';
      morphCard.style.backgroundColor = '';
      morphCard.style.borderColor = '';
      morphDoc.style.opacity = '';
      morphDoc.style.transform = '';
      morphCash.style.opacity = '';
      morphCash.style.transform = '';
      morphCash.style.pointerEvents = '';
      morphCashSum.style.filter = '';
      morphRows.forEach(function (r) { r.style.transform = ''; r.style.opacity = ''; });
      // В статичном варианте оба состояния уже на экране, поэтому подпись
      // говорит о разнице, а не отсчитывает дни
      morphCaption.textContent = morphScene.classList.contains('static')
        ? 'Вместо ' + DAYS + ' ' + daysGen(DAYS) + ' ожидания — деньги сегодня'
        : 'Оплата покупателя — через ' + DAYS + ' ' + daysWord(DAYS);
    };

    var morphRender = function () {
      morphTicking = false;
      if (morphScene.classList.contains('static')) return;
      var rect = morphScene.getBoundingClientRect();
      var total = rect.height - window.innerHeight;
      var p = total > 0 ? clampM(-rect.top / total) : 0;

      // Геометрия: портретный лист → альбомная плашка выплаты
      var g = easeM(clampM((p - 0.14) / 0.62));
      morphCard.style.width = Math.round(size.pw + (size.lw - size.pw) * g) + 'px';
      morphCard.style.height = Math.round(size.ph + (size.lh - size.ph) * g) + 'px';

      // Белая бумага → синие деньги; рамка уходит в тот же цвет
      var c = clampM((p - 0.34) / 0.3);
      var mix = function (a, b) { return Math.round(a + (b - a) * c); };
      var bg = 'rgb(' + mix(255, 35) + ',' + mix(255, 83) + ',' + mix(255, 232) + ')';
      morphCard.style.backgroundColor = bg;
      morphCard.style.borderColor = c > 0.06 ? bg : '';

      // Строки документа втягиваются одна за другой
      morphRows.forEach(function (r, i) {
        var k = clampM((p - (0.16 + i * 0.055)) / 0.18);
        r.style.transform = 'scaleX(' + (1 - k) + ')';
        r.style.opacity = 1 - k;
      });

      /* Передача, а не наложение: документ уходит вверх и гаснет, выплата
         поднимается снизу. Перехлёст короткий, иначе на экране одновременно
         две суммы и это читается как сбой. Сумма выплаты «наводится на
         резкость» из того же размытия, что и на счёте, — это тот же счёт. */
      var docT = clampM((p - 0.3) / 0.2);
      var cashT = clampM((p - 0.46) / 0.18);
      morphDoc.style.opacity = 1 - docT;
      morphDoc.style.transform = 'translateY(' + (-12 * docT) + 'px)';
      morphCash.style.opacity = cashT;
      morphCash.style.transform = 'translateY(' + (16 * (1 - cashT)) + 'px)';
      morphCashSum.style.filter = cashT < 1 ? 'blur(' + (7 * (1 - cashT)).toFixed(2) + 'px)' : '';
      morphCash.style.pointerEvents = cashT > 0.9 ? 'auto' : 'none';

      var left = Math.round(DAYS * (1 - clampM((p - 0.12) / 0.52)));
      morphCaption.textContent = left > 0
        ? 'Оплата покупателя — через ' + left + ' ' + daysWord(left)
        : 'Выплата Flott — деньги сегодня';
    };

    var morphRequest = function () {
      if (!morphTicking) { morphTicking = true; requestAnimationFrame(morphRender); }
    };

    var morphSub = document.getElementById('morphSub');
    var morphStage = morphScene.querySelector('.morph-stage');
    var morphHead = morphScene.querySelector('.morph-head');
    // Габариты обоих состояний; пересчитываются под высоту экрана
    var size = { pw: 330, ph: 430, lw: 560, lh: 240 };

    /* Карточка должна поместиться под заголовком, который теперь остаётся на
       экране. Считаем свободную высоту и от неё — портретный лист; альбомная
       плашка выводится из той же ширины. */
    var morphSizes = function () {
      var free = morphStage.clientHeight - morphHead.offsetHeight -
        morphCaption.offsetHeight - 36;
      var ph = Math.max(280, Math.min(430, Math.round(free)));
      var pw = Math.round(ph * 330 / 430);
      var lw = Math.min(560, Math.round(morphStage.clientWidth * 0.86));
      size = { pw: pw, ph: ph, lw: lw, lh: Math.round(lw * 240 / 560) };
    };

    var morphMeasure = function () {
      var isStatic = mqMorph.matches || morphReduce;
      morphScene.classList.toggle('static', isStatic);
      // Приглашение пролистать имеет смысл только там, где есть что листать
      if (morphSub) {
        morphSub.textContent = isStatic
          ? 'Тот же счёт № FL-4412: ожидание — или деньги сегодня.'
          : 'Тот же счёт № FL-4412 — просто пролистайте.';
      }
      if (isStatic) { morphClear(); return; }
      morphSizes();
      morphRender();
    };

    window.addEventListener('scroll', morphRequest, { passive: true });
    window.addEventListener('resize', morphMeasure);
    // Позиция секции может уехать после загрузки шрифтов и видео — без этого
    // карточка осталась бы в состоянии, посчитанном до сдвига
    window.addEventListener('load', morphMeasure);
    if (mqMorph.addEventListener) mqMorph.addEventListener('change', morphMeasure);
    morphMeasure();
  }
